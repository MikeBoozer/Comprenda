...
**Validity note.** This describes the *current* (pre-reconciliation) state. The repo and the deployed app are diverged (see [ADR-0002](decisions/0002-reconcile-workspace-repo-divergence.md)). After that reconciliation, parts of this runbook (especially "the repo is not a clean deploy source") will change — update this doc at that milestone.
...
| Tree | Location | Role |
|------|----------|------|
| **Local git repo** | `streamlit/` in this repo | Source you edit in your IDE. **Editing it does NOT change the live app.** Its home file (`nuance_app.py`) currently *lacks* the SPCS warehouse fix and uses `environment.yml` instead of `pyproject.toml` — so it is **not** a clean wholesale deploy source yet. |
| **Snowsight workspace** | `snow://workspace/"USER$"."PUBLIC"."DEFAULT$"/versions/live/nuance_app/` | What you edit in the Snowsight UI. **Editing it does NOT change the live app either.** |
...
- **Deploy button missing in Snowsight.** Expected after a main-file rename. Use the CLI sequence above. Reloading/viewing the app is just the **browser** (refresh / reopen the URL) — it never needed the Deploy button.
